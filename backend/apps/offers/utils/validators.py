...
def validate_salary(value):
    if not re.match(r"\d+(\s*-\s*\d+)?$", value):
        raise ValidationError(_("Wynagrodzenie powinno być w formie pojedynczej kwoty bądź widełek płacowych."))
...
# Requiremnt
def validate_unique_requirement(object, model):
    existing = model.objects.filter(
        offer=object.offer,
        skill=object.skill,
    ).exclude(id=object.id)
    if existing.exists():
        raise ValidationError(_("Wybrana umiejętność już istnieje dla tej oferty."))
...
def validate_mark_range(value):
    if value is not None and (value < 1 or value > 5):
        raise ValidationError(_("Ocena kompetencji powinna być w przedziale pomiędzy 1 a 5."))
