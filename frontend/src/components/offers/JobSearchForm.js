...
const JobSearchForm = () => {
  ...
  const handleCheckboxChange = e => {
    ...
    setSearchParams(prevSearchParams => {
      ...
      if (name === 'working_mode') {
        if (checked) {
          newSearchParams.working_mode = [...newSearchParams.working_mode, value];
        } else {
          newSearchParams.working_mode = newSearchParams.working_mode.filter(mode => mode !== value);
        }
      } else if (name === 'contract_type') {
        if (checked) {
          newSearchParams.contract_type = [...newSearchParams.contract_type, value];
        } else {
          newSearchParams.contract_type = newSearchParams.contract_type.filter(type => type !== value);
        }
      } else if (name === 'working_time') {
        if (checked) {
          newSearchParams.working_time = [...newSearchParams.working_time, value];
        } else {
          newSearchParams.working_time = newSearchParams.working_time.filter(time => time !== value);
        }
      } else if (name === 'category') {
        if (checked) {
          newSearchParams.category = [...newSearchParams.category, Number(value)];
        } else {
          newSearchParams.category = newSearchParams.category.filter(cat => cat !== Number(value));
        }
      }
  ...
  return (
    <>
    {Object.keys(categories).length === 0 ? <Loader /> : (
      ...
      <form onSubmit={handleSubmit}>
        <div className="row">
          ...
          <div className="col-lg-3 col-md-3 col-sm-12 mx-2">
            <button type="submit" className="btn btn-warning rounded-pill fw-bold w-100">Wyszukaj</button>
          </div>
          <div className="d-flex justify-content-center">
            <button type="button" className="btn btn-outline-secondary rounded-pill my-4 col-4" onClick={handleFiltersToggle}>
              Filtrowanie zaawansowane {isFiltersOpen ? <IoIosArrowUp /> : <IoIosArrowDown /> } 
            </button>
          </div>
        </div>
        <AdvancedFilters
