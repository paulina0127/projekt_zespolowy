...
const CandidateCourse = () => {
  ...
  const courseList = useSelector(state => state.userProfileDetails.courseList)
  ...
  const th_list = ['Nazwa','Opis', 'Data rozpoczęcia', 'Data zakończenia', 'Akcje']
    ...
    <UserPanelLayout>
      <div className='container mt-3'>
        ...
        <CandidateTable
          ...
          th_list={th_list}
        > 
        {!loading && courseList && courseList.results.map((course, index) => 
        <tr key={course.id}>
          <td>{course.name}</td>
          ...
          <MyModal showModal={true} title='Usuwanie kursu' danger={true}>
            <CandidateInfoDelete
              name='ten kurs'
              id={course.id}
